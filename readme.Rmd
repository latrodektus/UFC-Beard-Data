...
```{r}
winner$facehair[winner$facehair==2]<-1
loser$facehair[loser$facehair==2]<-1
beards2 <- set_up_btm(predictors, winner, loser)
...
model3<-BTm(player1=winner,player2=loser,
            formula = ~  as.factor(prev) + as.factor(facehair)  + ht[ID] + reach[ID] + stance[ID] + (1|ID), id="ID",data=b.out)
summary(model3)
```
